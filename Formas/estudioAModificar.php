<?php 
	...
	$query = "SELECT nombrePosgrado FROM estudio WHERE codigo = $codigo";
	...
	{
		$html = $html.'<form class="form-horizontal" role="form" action="modificarEstudio.php" method="post">
	<label>Estudios</label>';
		...
		while(($row = mysqli_fetch_assoc($resultadoConsulta)) != NULL){
			$html = $html.'
		        ...
		        <label>
                        <input required type="radio" name="estudioSeleccionado" value="'.$row["nombrePosgrado"].'">'.$row["nombrePosgrado"].'</label>
	                </div>';
		}
		$html = $html.'
	...
	else
	{
		...
		<h1>El maestro no tiene estudios</h1>
		</div>';
	}
	$html = $html.'</div>';
	echo $html;
