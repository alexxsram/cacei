<?php 
	...
	{
		$html = $html.'<form class="form-horizontal" role="form" action="eliminarProgramaHabilidad.php" method="post">
	<label>Programas</label>';
		...
		while(($row = mysqli_fetch_assoc($resultadoConsulta)) != NULL){
			$html = $html.'
		        ...
		        <label>
                        <input type="checkbox" name="'.eliminaEspacioEnBlanco($row["programa"]).'" value="S">'.$row["programa"].'</label>
	                </div>';
		}
		$htlm = $html.'<div class="col-sm-offset-2 col-sm-10">
