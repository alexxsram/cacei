  ...
  $('#deleteReporte').on('show.bs.modal', function (event) {
    ...
    var button = $(event.relatedTarget)
    var modal = $(this)
    var titulo = $("#h2Reporte")
    var buttonD = $("#btnReporte")
    buttonD.show()
    titulo.html("Se eliminará el reporte: " + button.data('title'))
    $("#idReporteDR").val( button.data("idreporte") )
    $( "#deleteReporteF" ).submit(function( event ) {
    			 $.ajax({
    					type: "POST",
    					url: "model/delete_reporte.php",
              dataType: "HTML",
    					data: "id=" + $("#idReporteDR").val()
    			}).done(function (echo){
            if(echo == '1'){
              mensajeInfo()
                mensajeEdit.html("El reporte fue eliminado con éxito")
                buttonD.hide()
                $("#btnReporteC").html("Cerrar")
                cargar()
                mensajeEliminado()
            }else{
              mensajeAlerta()
